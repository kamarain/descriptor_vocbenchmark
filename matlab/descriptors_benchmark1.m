% DESCRIPTORS_BENCHMARK1 VOC benchmark descriptors
%
% Runs the descriptor benchmark described in ref. [2] (match
% criterion changed to ref. [1]). Evaluation is based on the
% configuration file settings in DESCRIPTOR_BENCHMARK1_CONF.M .
%
% References
...
%
% See also DESCRIPTORS_BENCHMARK1_CONF.M and DETECTORS_BENCHMARK1.M .

% Clear display and start timing
%clear;
clearvars -except confParameter timeFunction r q w t y;
close all;
tic

% 0. Config & Initialisation
% Run the configuration function
conf = descriptors_benchmark1_conf();
  ...
  for imgInd = 1:imgTot
    ...
    tic;
      ...
      if conf.enforceFeatures == true || ~exist(fullfile(fSavePath,fSaveFileA), 'file') ...
        ...
        sgS = []; % provided only by the simple Gabor descriptor

        if strcmp(conf.detdesc{detdescNo},'vl_dense+mvpr_sg')
          [framesA descriptorsA sgS configures] = ...
              mvpr_feature_extract_new(imgA, ...
                                       'method', conf.detdesc{detdescNo},...
                                       'debugLevel',conf.debugLevel);
          [framesB descriptorsB sgS configures] = ...
              mvpr_feature_extract_new(imgB, ...
                                       'method', conf.detdesc{detdescNo},...
                                       'debugLevel',conf.debugLevel);
        else % no sgS returned
        ...
        save(fullfile(fSavePath,fSaveFileA),'frames', 'descriptors','sgS');
        frames = framesB; descriptors = descriptorsB;
        save(fullfile(fSavePath,fSaveFileB),'frames', 'descriptors','sgS');
      end;
      timeElapsed = toc;
    ...
    for imgInd = 1:imgTot
      ...
      tic;
        ...
        if conf.enforceFeatures == true || ~exist(fullfile(fSavePath,fSaveFileA_new), 'file') ...
          ...
          load(fullfile(fSavePath,fSaveFileA),'frames', 'descriptors','sgS');
          ...
          % Save frames (detected regions) and their descriptors
          frames = framesA; descriptors = descriptorsA;
          save(fullfile(fSavePath,fSaveFileA_new),'frames', 'descriptors','sgS');
          frames = framesB; descriptors = descriptorsB;
...
for detdescNo = 1:length(conf.detdesc)
  ...
  matchSaveFile = ['SAVE_descriptors_benchmark1_' ...
  ...
  if exist(fullfile(matchSaveDir,matchSaveFile),'file') && ~conf.enforceMatching
    load(fullfile(matchSaveDir,matchSaveFile),'rm','cm','real_matches','correct_matches','conf','imgInd');
    startInd = imgInd;
  ...
  for imgInd = 1:imgTot
    ...
    if startInd > imgInd 
      ...
      [erro_rep repeat_rep corresp_rep  match_score_rep matches_rep twi_rep corr_rep] =...
          mikolajczyk_repeatability(framesA,framesB,descriptorsA,descriptorsB,H,imgASz,imgBSz,0);
      
      % Take the (logical) match matrix of 50 overlap
      correspondences = corr_rep{5};
      ...
      res.erro(imgInd)                = erro_rep(idx);
      res.repeatability(imgInd)       = repeat_rep(idx);
      res.n_correspondences(imgInd)   = corresp_rep(idx);
      res.match_score(imgInd)         = match_score_rep;
      res.n_matches(imgInd)           = matches_rep;
      res.twi{imgInd}                 = twi_rep;
      
      % Compute descriptor distances between each descriptor
      match_method = 'L2'; % default
      match_sgS = sgS;
      hamming = false;
      switch conf.detdesc{detdescNo}
       case 'cv_orb+cv_brief',
        hamming =  true;
       case 'cv_dense+cv_brief',
        hamming =  true;
       case 'vl_dense+mvpr_sg',
        match_method = 'sg';
       otherwise
        hamming = false;
      end;

      [sm] = mvpr_feature_matchmatrix(descriptorsA, descriptorsB, ...
                                      'hamming', hamming,...
                                      'sgS', match_sgS,...
                                      'method', match_method);
      ...
      % It could happen that no enough descriptors in image B
      real_bestMatchMax = min(conf.bestMatchMax,size(descriptorsB,2));
      
      % Take N best matches in the following format:
      %
      % [idxA_1 idxB_match1 .. idxB_matchN ;
      %  idxA_2 ...
      %  ...]
      matches = [(1:size(sm,1))' sidx(:,1:real_bestMatchMax)];

      %
      % MATCH COUNTING
      %
      rmInd = zeros(real_bestMatchMax,1);
        
        % Check matches
        for i=1:size(matches,1) % AGB: i counts source indices in B
                                % AGB: Take all matching feature points from A
          indMatch = matches(i,2:end);
          
          for j=1:real_bestMatchMax % AGB: j counts all bestMatchMax matches of i in B
            jj = indMatch(j); % Corresponding index
                              % AGB: Test the feature correspondence using either overlap
                              % criterion or centroid distance criterion
            correct_matches{imgInd}(j,i) = false;
            if conf.descMatchCriterion == 1
              correct_matches{imgInd}(j,i) = logical( correspondences(jj, i) );
              %                 correct_matches{imgInd}(j,i) = logical( correspondences(i,jj) );
            else
              % Calculate distances for point j
              spatialDistance = sqrt( sum( (LA(1:2,i)-FB(1:2,jj)).^2 ) );
              %                 spatialDistance = sqrt( sum( (LA(1:2,jj)-FB(1:2,i)).^2 ) );
              correct_matches{imgInd}(j,i) = logical( spatialDistance < spatialThreshold );
            end
            
            % AGB: If correspondence test turned out true, store
            if correct_matches{imgInd}(j,i)
              if conf.debugLevel > 1
                fprintf('\tAGB: Correct feature correspondence found for match A --> B: %d --> %d\n',...
                        i,jj);
                %                 fprintf('\tAGB: Correct feature correspondence found for match A --> B: %d --> %d\n',...
                %                     jj,i);
              end
            ...
            else % else if correct_matches{imgInd}(j,i)
              if conf.debugLevel > 1
                fprintf('\t\tAGB: Incorrect feature correspondence found for match %d --> %d\n',...
                        i,jj);
                %                     fprintf('\t\tAGB: Incorrect feature correspondence found for match A --> B: %d --> %d\n',...
                %                         jj,i);
              end
            end % end if correct_matches{imgInd}(j,i)
            
          end % for j (bestMatchMax matches)
          
        end % for i (source indices in A)

        % AGB: Report number of matches to the total number of features
        % AGB: parfor-crap
        sumrmInd = sum(rmInd);
       % fprintf('  [%d correct matches out of %d corresp. (%d/%d tot feats in A/B)\n',...
       %         sumrmInd, sum(sum(correspondences)), size(descriptorsA,2), size(descriptorsB,2));
       % for i=1:numel(rmInd)
       %   fprintf('    %d. %d matches\n', i,rmInd(i));
       % end
        rmInd = cumsum(rmInd); % accumulate
        real_matches(1:real_bestMatchMax,imgInd) = rmInd;
        rm{detdescNo} = real_matches;
        cm{detdescNo} = correct_matches;
        
        %save(fullfile(matchSaveDir,matchSaveFile),'real_matches','correct_matches','conf','imgInd');    
        save(fullfile(matchSaveDir,matchSaveFile),'rm','cm','real_matches','correct_matches','conf','imgInd');    
        timeElapsed = toc;
        timeTotalElapsed = timeTotalElapsed + timeElapsed;
    end
  end;
...
mikolajczyk_matches_plot(classes,classnames, conf.detdesc, ...
                         rm, cm,...
                         ...
                         'bestMatchMax',conf.bestMatchMax,...
