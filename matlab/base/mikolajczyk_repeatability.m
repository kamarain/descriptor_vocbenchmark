% MIKOLAJCZYK_REPEATABILITY Mikolajczyk's repeatability test
%
%  This function is partly edited version of the original from:
%   http://www.robots.ox.ac.uk/~vgg/research/affine/
%
%  We have tuned it to work directly with our functionality where
%  detectors and descriptors basically follow the VL_FEAT formats.
%
%function [erro,repeat,corresp, match_score,matches, twi, correspondences]=myrepeatability(file1,file2,Hom,imf1,imf2, common_part)
function [erro,repeat,corresp, match_score,matches, twi, correspondences]=...
    mikolajczyk_repeatability(framesA,framesB,descriptorsA, ...
                                  descriptorsB,Hom, imASize, imBSize, common_part)
% Comments out since bloody annoying in big runs
fprintf(' - mikolajczyk_repeatability: fprintf commented out -');
...
%dimdesc=dimdesc1;
if size(f1,1)==5 & size(f1,1)==size(f2,1) 
%  fprintf(1,', affine regs?');
else
  error('Dim=5 expected for frames');
end;

if (dimdesc1 ~= dimdesc2)
  error('Mismatch of descriptor dimensions.');
end;
dimdesc=dimdesc1;
...
feat1=zeros(9+dimdesc,numdesc1);
feat2=zeros(9+dimdesc,numdesc2);
feat1(1:5,1:numdesc1)=f1(1:5,1:numdesc1);
feat2(1:5,1:numdesc2)=f2(1:5,1:numdesc2);
if size(f1,1)>1
  feat1(10:9+dimdesc,1:numdesc1)=s1;
  feat2(10:9+dimdesc,1:numdesc2)=s2;
end;
...
%AGB
correspondences = cell(1,6);
for i=1:6,
wi=(wout<erro(i));
corresp(i)=sum(sum(wi));
% AGB
correspondences{i}=wi;
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
